  ...
  ({ label, error, helperText, className = '', ...props }, ref) => {
    return (
      <div className="w-full">
        ...
        <textarea
          ref={ref}
          className={`w-full min-h-24 px-5 py-3 bg-[var(--panel-2)] border border-[var(--border)] text-[var(--text)] placeholder-[var(--text-muted)] rounded-[var(--radius-control)] transition-colors focus:outline-none focus:border-[var(--border-active)] focus:ring-2 focus:ring-[var(--ring)] disabled:bg-[var(--panel)] disabled:text-[var(--text-disabled)] disabled:cursor-not-allowed resize-none ${error ? 'border-[var(--danger)]' : ''} ${className}`}
          {...props}
        />
        {error && <p className="text-xs text-[var(--danger)] mt-1">{error}</p>}
        {helperText && !error && <p className="text-xs text-[var(--text-muted)] mt-1">{helperText}</p>}
