...
/**
 * BlackboardSyncManager - Handles automatic Blackboard LMS synchronization
 *
 ...
 */
export function BlackboardSyncManager() {
  ...
  const performSync = useCallback(async (reason: string) => {
    ...
    if (syncInProgress.current) {
      console.log('[BlackboardSync] Sync already in progress, skipping');
      return;
    }

    const now = Date.now();
    if (now - lastSyncAttempt.current < MIN_SYNC_GAP) {
      console.log('[BlackboardSync] Too soon since last sync attempt, skipping');
      return;
    }

    try {
      // Check Blackboard connection status
      const statusRes = await fetch('/api/blackboard/status');
      if (!statusRes.ok) {
        console.log('[BlackboardSync] Failed to fetch Blackboard status');
        return;
      }

      const statusData = await statusRes.json();
      if (!statusData.connected || !statusData.syncEnabled) {
        console.log('[BlackboardSync] Blackboard not connected or sync disabled');
        return;
      }
      ...
      if (reason === 'visibility' && timeSinceLastSync < STALE_THRESHOLD) {
        console.log(`[BlackboardSync] Last sync was ${Math.round(timeSinceLastSync / 60000)} minutes ago, skipping (${reason})`);
        return;
      }

      console.log(`[BlackboardSync] Starting sync (${reason}), last sync was ${Math.round(timeSinceLastSync / 60000)} minutes ago`);

      syncInProgress.current = true;
      lastSyncAttempt.current = now;

      // Perform the sync
      const syncRes = await fetch('/api/blackboard/sync', {
        method: 'POST',
      ...
      if (syncRes.ok) {
        ...
        console.log('[BlackboardSync] Sync completed:', result);
        ...
        console.error('[BlackboardSync] Sync failed:', await syncRes.text());
      }
    } catch (error) {
      console.error('[BlackboardSync] Sync error:', error);
    } finally {
