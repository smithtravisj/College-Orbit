...
/**
 * BrightspaceSyncManager - Handles automatic Brightspace (D2L) LMS synchronization
 *
 ...
 */
export function BrightspaceSyncManager() {
  ...
  const performSync = useCallback(async (reason: string) => {
    ...
    if (syncInProgress.current) {
      console.log('[BrightspaceSync] Sync already in progress, skipping');
      return;
    }

    const now = Date.now();
    if (now - lastSyncAttempt.current < MIN_SYNC_GAP) {
      console.log('[BrightspaceSync] Too soon since last sync attempt, skipping');
      return;
    }

    try {
      // Check Brightspace connection status
      const statusRes = await fetch('/api/brightspace/status');
      if (!statusRes.ok) {
        console.log('[BrightspaceSync] Failed to fetch Brightspace status');
        return;
      }

      const statusData = await statusRes.json();
      if (!statusData.connected || !statusData.syncEnabled) {
        console.log('[BrightspaceSync] Brightspace not connected or sync disabled');
        return;
      }
      ...
      if (reason === 'visibility' && timeSinceLastSync < STALE_THRESHOLD) {
        console.log(`[BrightspaceSync] Last sync was ${Math.round(timeSinceLastSync / 60000)} minutes ago, skipping (${reason})`);
        return;
      }

      console.log(`[BrightspaceSync] Starting sync (${reason}), last sync was ${Math.round(timeSinceLastSync / 60000)} minutes ago`);

      syncInProgress.current = true;
      lastSyncAttempt.current = now;

      // Perform the sync
      const syncRes = await fetch('/api/brightspace/sync', {
        method: 'POST',
      ...
      if (syncRes.ok) {
        ...
        console.log('[BrightspaceSync] Sync completed:', result);
      } else {
        console.error('[BrightspaceSync] Sync failed:', await syncRes.text());
      }
    } catch (error) {
      console.error('[BrightspaceSync] Sync error:', error);
    } finally {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      performSync('initial');
    }, 3000); // Slightly staggered from other sync managers
