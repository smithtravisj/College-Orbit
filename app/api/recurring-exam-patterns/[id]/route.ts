...
import { generateRecurringExamInstances } from '@/lib/recurringExamUtils';

// PATCH update a recurring exam pattern
export async function PATCH(
  ...
  try {
    ...
    console.log('[PATCH /api/recurring-exam-patterns/[id]] Received data:', JSON.stringify(data, null, 2));
    ...
    console.log('[PATCH /api/recurring-exam-patterns/[id]] Existing pattern:', {
    ...
    const updated = await prisma.recurringExamPattern.update({
      where: { id },
      data: {
        ...
        examTemplate:
          'examTemplate' in data ? data.examTemplate : pattern.examTemplate,
      },
    });

    console.log('[PATCH /api/recurring-exam-patterns/[id]] Updated pattern:', {
    ...
    console.log('[PATCH /api/recurring-exam-patterns/[id]] Deleting old instances and regenerating...');
    try {
      ...
      console.log('[PATCH /api/recurring-exam-patterns/[id]] Successfully regenerated instances');
    } catch (error) {
      console.error('[PATCH /api/recurring-exam-patterns/[id]] Error regenerating instances:', error);
    }

    return NextResponse.json({ pattern: updated });
  } catch (error) {
    console.error('[PATCH /api/recurring-exam-patterns/[id]] Error:', error);
