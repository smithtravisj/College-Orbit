...
// PATCH update a recurring deadline pattern
export async function PATCH(
  ...
  try {
    ...
    console.log('[PATCH /api/recurring-deadline-patterns/[id]] Received data:', JSON.stringify(data, null, 2));
    ...
    console.log('[PATCH /api/recurring-deadline-patterns/[id]] Existing pattern:', {
    ...
    const updated = await prisma.recurringDeadlinePattern.update({
      where: { id },
      data: {
        ...
        deadlineTemplate:
          'deadlineTemplate' in data ? data.deadlineTemplate : pattern.deadlineTemplate,
      },
    });

    console.log('[PATCH /api/recurring-deadline-patterns/[id]] Updated pattern:', {
    ...
    console.log('[PATCH /api/recurring-deadline-patterns/[id]] Deleting old instances and regenerating...');
    try {
      ...
      console.log('[PATCH /api/recurring-deadline-patterns/[id]] Successfully regenerated instances');
    } catch (error) {
      console.error('[PATCH /api/recurring-deadline-patterns/[id]] Error regenerating instances:', error);
    }

    return NextResponse.json({ pattern: updated });
  } catch (error) {
    console.error('[PATCH /api/recurring-deadline-patterns/[id]] Error:', error);
