...
import { Monitor, RefreshCw, Link2, Unlink, ChevronDown, AlertCircle, Chrome, ExternalLink, Music } from 'lucide-react';
    ...
    <>
      ...
      <div className="mx-auto w-full max-w-[1800px]" style={{ padding: 'clamp(12px, 4%, 24px)', paddingTop: '0', position: 'relative', zIndex: 1 }}>
        ...
        <div className="w-full" style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit, minmax(400px, 1fr))', gap: isMobile ? '14px' : 'var(--grid-gap)', maxWidth: '100%', boxSizing: 'border-box' }}>
          ...
          {/* Spotify Integration - Full width section */}
          <div style={{ gridColumn: '1 / -1' }}>
...
function SpotifyIntegrationCard({ isMobile, theme }: { isMobile: boolean; theme: 'light' | 'dark' }) {
  ...
  const { isPremium: isAppPremium } = useSubscription();
  const {
    isConnected,
  ...
  // These are kept for when Spotify re-enables new app creation
  const [_isConnecting, setIsConnecting] = useState(false);
  const [isDisconnecting, setIsDisconnecting] = useState(false);
  const [message, setMessage] = useState('');
  void _isConnecting; // Suppress unused variable warning
